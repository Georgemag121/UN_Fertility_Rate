...
tfr_pred1 <- tfr %>% mutate(year = year(TimeMid), Country.or.area = as.character(Country.or.area)) %>% left_join(., pred.table, by = c("Country.or.area" = "Country.or.area", "year" = "year")) %>% mutate(res = DataValue - predict) %>% group_by(DataProcessType, region, development) %>% mutate(avg.adj1 = -mean(res)) %>% ungroup() %>% mutate(adjd.tfr = DataValue + avg.adj1)
...
tfr_pred2 <- tfr1950 %>% mutate(Country.or.area = as.character(Country.or.area)) %>% left_join(., pred.table2, by = c("Country.or.area" = "Country.or.area", "year" = "year")) %>% mutate(res = DataValue - predict) %>% group_by(DataProcessType, region, development) %>% mutate(avg.adj1 = -mean(res)) %>% ungroup() %>% mutate(adjd.tfr = DataValue + avg.adj1)

tfr_pred2_plot <- function(country) {
  tfr_pred2 %>% filter(Country.or.area == country) %>% ggplot() + geom_point(aes(x = year, y = DataValue), col = "blue") + geom_smooth(aes(x= year, y = DataValue)) + geom_point(aes(x= year, y = adjd.tfr), col = "red") + geom_smooth(aes(x= year, y = adjd.tfr), col = "red") + ggtitle(paste(country))
}
